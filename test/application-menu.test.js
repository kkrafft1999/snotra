...
test('macOS: Einstellungen stehen im App-Menue direkt unter „Ueber“', () => {
  ...
  assert.deepEqual(labels.slice(0, 5), ['about', 'separator', 'Einstellungen\u2026', 'separator', 'services']);
});

test('macOS: Ansicht traegt die Einstellungen nicht mehr', () => {
  const { template } = buildTemplate('darwin');
  const view = menuNamed(template, 'Ansicht');
  assert.ok(view, 'das Menue Ansicht existiert weiter');
  assert.ok(!view.submenu.some((item) => item.label === 'Einstellungen\u2026'));
  // Der Rest der Ansicht bleibt unangetastet: Seitenleiste oben, dann Neu laden.
  assert.equal(view.submenu[0].label, 'Seitenleiste ein-/ausblenden');
...
for (const platform of ['win32', 'linux']) {
  test(`${platform}: Einstellungen stehen in Ansicht, es gibt kein App-Menue`, () => {
    ...
    assert.equal(template[0].label, 'Bearbeiten', 'ohne App-Menue beginnt die Leiste mit Bearbeiten');

    const view = menuNamed(template, 'Ansicht');
    const labels = view.submenu.map(labelOf);
    assert.deepEqual(labels.slice(0, 5), [
      'Seitenleiste ein-/ausblenden', 'separator', 'Einstellungen\u2026', 'separator', 'Neu laden',
    ]);
  });
}
...
for (const platform of ['darwin', 'win32', 'linux']) {
  test(`${platform}: Einstellungen genau einmal, mit CmdOrCtrl+,`, () => {
    const { template } = buildTemplate(platform);
    const items = allItems(template).filter((item) => item.label === 'Einstellungen\u2026');
...
test('Einstellungen schicken UI_OPEN_SETTINGS ans Fenster', () => {
  ...
  const settings = allItems(template).find((item) => item.label === 'Einstellungen\u2026');
  settings.click();
...
test('ohne Fenster laeuft der Klick ins Leere statt zu werfen', () => {
  ...
  assert.doesNotThrow(() => items.find((item) => item.label === 'Einstellungen\u2026').click());
  assert.doesNotThrow(() => items.find((item) => item.label === 'Seitenleiste ein-/ausblenden').click());
});
...
test('Hilfe: Update-Pruefung und GitHub-Link haengen an den Callbacks', () => {
  ...
  const help = menuNamed(template, 'Hilfe');
  assert.equal(help.role, 'help');
  help.submenu.find((item) => item.label.startsWith('Nach Updates suchen')).click();
